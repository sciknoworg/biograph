...
| [`jsonschema`](https://pypi.org/project/jsonschema/) (Python), `Draft202012Validator` + `RefResolver` | Validates every subject file against its schema, then a second pass checks referential integrity by hand — every `participants[].entity_id`, `location`, `source_id`, and `event_id` referenced must actually exist in that subject's files, which JSON Schema alone can't express. That pass also enforces each relation type's fixed reading direction via a hand-maintained `RELATION_DIRECTIONS` table (`worked_at` really does need a `person` source and an `organization` target, not just ids that resolve to *something*) — JSON Schema can express "these fields are entity ids" but not "and this one must be a person." |
